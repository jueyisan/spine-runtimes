...
if [ "$os" == "macos" ] && [ $dev == "false" ]; then
	...
	scons $target $mono_module arch=x86_64 compiledb=yes custom_modules="../spine_godot" opengl3=yes --jobs=$cpus use_embree=no
	scons $target $mono_module arch=arm64 compiledb=yes custom_modules="../spine_godot" opengl3=yes --jobs=$cpus use_embree=no
	if [ $mono == "true" ]; then
